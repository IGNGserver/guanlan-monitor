...
- Windows setup, Windows update ZIP, Linux Debian package, and Android APK must
  come from a GitHub Release asset generated by GitHub Actions.
- Unattended installations must use those same versioned assets. The Windows
  setup is driven with `/S` and the documented configuration switches; the Linux
  package is driven with `dpkg`/`apt` plus `guanlan-agent config set`. Neither
  path may download binaries from `main` or build on the target machine.
...
non-interactive configuration and status checks.
