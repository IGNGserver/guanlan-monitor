...
> 历史复核记录：本表记录 2026-08-28 的状态。虚拟机采集及相关产品功能已在 `v3.0.87` 移除；旧条目中的虚拟化描述仅保留审计历史，不代表当前行为。
...
| F-08 | 真实：设备删除后，旧实时/缓存状态可能重新制造 open 设备 | 已修复并与 UI 语义对齐：MySQL/local 写入 closed tombstone，删除时清实时态；历史/缓存不能复活，只有后续被接受的 Agent 上报才能重新显示设备 |
