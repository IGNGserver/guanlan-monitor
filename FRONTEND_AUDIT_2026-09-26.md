...
1. **范围**：原建议在「只做 P0/P1」与「连 P2/P3 一起」之间选。结论是全做，包括 Context 重构与全量术语替换。事后看，这个选择的价值集中在批次 3：如果只修视觉，那 6 处热区退化仍然会在，而且仍然没有任何测试能看见它们。
2. **发布节奏**：原建议在「合成一版」与「每批一版」之间选。结论是合成一版（v3.0.110），因为 6 个批次改的是同一批样式文件和同一个 Context，分批只会在同一处反复解冲突；CI 是唯一的构建门，一版一次全量门禁也更可信。

线上中枢当时是 `3.0.106`，HEAD 是 `3.0.108`，中间两版未部署——这也是为什么审计阶段必须把「线上实测」和「HEAD 源码」当成两条独立证据，不能拿线上的渲染结果直接当 HEAD 的结论。最终 v3.0.110 经 `deploy-test.yml` 部署、复验，v3.0.112 收掉最后一处几何例外。
