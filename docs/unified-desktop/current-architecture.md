...
- MySQL: `device_minute_metrics` retains 90 days and `device_hourly_metrics` retains 370 days; instance data and recorded detail metadata are JSON columns. The former `widget_layout_instances` / `widget_layout_templates` tables were dropped in v3.0.104 together with the widget mechanism; their contents were exported and verified before the drop.
