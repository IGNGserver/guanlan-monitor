...
> 已归档的历史计划：本文的功能清单和验收场景基于虚拟机功能尚未移除时的产品状态。虚拟机采集、API、模型和 UI 已在 `v3.0.87` 移除；本文仅供追溯当时的重构决策，不作为当前功能要求。
>
> 小组件机制（Widget 布局、自定义面板、展板模式与远程布局保存）已在 `v3.0.104` 整体移除，设备详情改为前端常量驱动的固定 Carbon 图表布局；本文所有涉及 Widget 的条目仅作历史追溯。
