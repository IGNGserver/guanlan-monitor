...
状态：基础层和窄屏外壳已落地；第二阶段正在迁移高频控件，页面按后续阶段推进。
...
2. 应用外壳：标题栏、导航、顶部栏、搜索、Snackbar、Dialog 和移动端导航。
   当前已完成窄屏 Navigation Bar、移动端 Modal Drawer 边界、设备详情 Tabs，以及时间范围、刷新频率、主题/密度、开关、Snackbar、命令搜索和 Dialog 输入等高频控件的 Material 3 交互迁移。
