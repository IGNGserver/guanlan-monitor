...
状态：基础层、窄屏外壳、高频控件、页面级状态摘要和设备详情状态面已落地；设置/登录与最终多端验收按后续阶段推进。
...
2. 应用外壳：标题栏、导航、顶部栏、搜索、Snackbar、Dialog 和移动端导航。
   当前已完成窄屏 Navigation Bar、移动端 Modal Drawer 边界、设备详情 Tabs，以及时间范围、刷新频率、主题/密度、开关、Snackbar、命令搜索、Dialog 输入、筛选 Select、Checkbox、硬件探针配置、小组件抽屉操作、布局编辑器工具栏、总览状态摘要、设备详情状态面和无遥测反馈的 Material 3 交互迁移。
